...
def make_model(df, features_list, dum_cols):
    ...
    plt.title('Model Residuals', size=40)
    plt.ylabel('Count', size=25)
    plt.xticks(size=20)
    plt.yticks(size=20)
    plt.xlabel('Difference Between Actual and Predicted Sale Price', size=25)
    residuals.hist()
    plt.axvline(0, color = 'red')
    
    query = input('Save histogram? Please enter "yes" or "no".') #option to save hist with timestamp
    if query == 'yes':
        name = str(pd.Timestamp.now())
        namelist = []
        for l in list(name):
    ...
    #uery = input('Save histogram? Please enter "yes" or "no".')
    ...
    return f'Your RMSE for test {pd.Timestamp.now()} is {metrics.mean_squared_error(np.exp(y), np.exp(pred))**.5}.' #print RMSE with timestamp
